...
struct OpenAIProviderTests {
    ...
    func streamingChatSuccess() async throws {
        ...
        #expect(events.contains(.delta(requestID: requestID, text: "Hello")))
        #expect(events.contains(.completed(requestID: requestID)))
        let requestDebugSeen = events.contains { event in
            if case let .debug(id, info) = event {
                return id == requestID
                    && info.requestURL == "https://api.openai.com/v1/chat/completions"
                    && info.httpMethod == "POST"
            }
            return false
        }
        #expect(requestDebugSeen)
    }
    ...
    func nonContentDeltasIgnored() async throws {
        ...
        #expect(events.first == .started(requestID: requestID))
        #expect(events.contains(.delta(requestID: requestID, text: "Real")))
        #expect(events.contains(.completed(requestID: requestID)))
    }

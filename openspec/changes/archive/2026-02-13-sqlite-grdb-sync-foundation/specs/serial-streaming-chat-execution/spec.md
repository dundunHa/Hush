...
The system MUST persist accepted request outputs so terminal request results can be recovered after restart.

#### Scenario: Completed request persists assistant result
- **WHEN** an active request reaches completed terminal state
- **THEN** the system SHALL durably persist the associated assistant message content and terminal status

#### Scenario: Stopped or failed request preserves partial persisted output
- **WHEN** an active request stops or fails after one or more delta events
...
The system MUST preserve queue-full atomic rejection semantics for persisted data.

#### Scenario: Queue-full rejection does not create persisted message
- **WHEN** a submission is rejected because the pending queue has reached capacity

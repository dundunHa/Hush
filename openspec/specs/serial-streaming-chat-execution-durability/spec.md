...
The system MUST preserve queue-full atomic rejection behavior when persistence is enabled in concurrent multi-conversation execution.

#### Scenario: Queue-full rejection produces zero durable writes
- **WHEN** the user submits a message and the global queued capacity is at maximum
...
The system MUST durably persist the streaming assembly lifecycle without changing request terminal semantics, and MUST route by request ownership.
...
- **THEN** the system SHALL persist the user message record in the request’s owning conversation

#### Scenario: First delta creates one durable in-progress assistant message
- **WHEN** the first provider delta event is received for a running request
...
- **WHEN** additional provider delta events are received for a running request
